...
func init() {
	android.RegisterModuleType("heka_generator", GeneratorFactory)
}
...
func (g *Module) GenerateAndroidBuildActions(ctx android.ModuleContext) {
	...
	if depRoot == "" {
		...
		depRoot = hekaExpandVariables(ctx, depRoot)
	}

	// Glob dep_files property
	for _, dep_file := range g.properties.Dep_files {
		dep_file = hekaExpandVariables(ctx, dep_file)
	...
	exCmd := hekaExpandVariables(ctx, String(g.properties.Cmd))
	...
	manifestName := "heka.sbox.textproto"
	desc := "generate"
	name := "generator"
